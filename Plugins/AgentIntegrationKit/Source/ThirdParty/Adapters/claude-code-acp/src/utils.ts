...
/**
 * Normalize project cwd for cross-platform equality checks.
...
export function normalizeProjectCwd(cwd: string): string {
  ...
  const windowsPathMatch = slashNormalized.match(/^([A-Za-z]):\//);
  if (windowsPathMatch) {
    return `${windowsPathMatch[1].toUpperCase()}:${slashNormalized.slice(2)}`;
  }
  return slashNormalized;
}

// Helper to encode a path like Claude does:
// - Unix: "/Users/test dir" -> "-Users-test-dir"
// - Windows: "C:\Users\test dir" -> "C-Users-test-dir"
// Claude Code replaces both path separators AND spaces with hyphens.
export function encodeProjectPath(cwd: string): string {
  ...
  if (windowsPathMatch) {
    ...
    return `${driveLetter}${rest.replace(/[\\/\s]/g, "-")}`;
  }
